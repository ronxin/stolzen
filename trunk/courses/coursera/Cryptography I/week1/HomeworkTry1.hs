module HomeworkTry1 where

import Hex
import Data.Char
import Data.List
import Data.Bits

-- Homework 1

ct1  = ""
ct2  = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
ct3  = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
ct4  = ""
ct5  = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
ct6  = ""
ct7  = ""
ct8  = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
ct9  = ""
ct10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
targ = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

allCt = map hexByteStringToInt [ct1, ct2, ct3, ct4, ct5, ct6, ct7, ct8, ct9, ct10, targ]

---


eachWithEach :: (a -> a -> b) -> [a] -> [b]
eachWithEach _ [x] = []
eachWithEach f (x:xs) = (map (\el -> f x el) xs) ++ (eachWithEach f xs)

upper :: Int -> Bool
upper i = c >= 'A' && c <= 'Z' where c = chr i

lower :: Int -> Bool
lower i = c >= 'a' && c <= 'z' where c = chr i

space :: Int -> Bool 
space i = c == ' ' where c = chr i

letter :: Int -> Bool
letter i = (lower i) || (upper i)


intSpace :: Int
intSpace = ord ' '

--    where xored = (c1 `listXor` c2) 


-- idea:
-- when we compute c1 xor c2, we get m1 xor m2
-- next,
-- when we happened to xor with ' ' (space), the letter in original message remains the same letter, but 
-- with different case (ie. 'a' -> 'A' and 'A' -> 'a')
-- so, if we see something that looks like a good printable English letter, 
    -- chances are that this is due to xor with a space
-- it's possible that key is either [c1 xor ' '] or [c2 xor ' '] - so keep both


computeKeyLetter :: Int -> Int -> (Int, Int)
computeKeyLetter c1 c2
          | letter (c1 `xor` c2) =
                (c1 `xor` intSpace, c2 `xor` intSpace)
          | otherwise = (0, 0)

-- applies computeKeyLetter to each element of the list
computeKey :: [Int] -> [Int] -> ([Int], [Int])
computeKey ct1 ct2 = unzip (map2 computeKeyLetter ct1 ct2)

-- first set of keys when first message xored with ' ', second - when second
(keys1, keys2) = unzip (eachWithEach computeKey allCt)


-- now need to compute whole key

testData :: [[Int]]
testData = [[1, 0, 2, 2, 0], [0, 0, 3, 5, 0], [0, 0, 0, 1, 0]]



filterOrZero :: [Int] -> [Int]
filterOrZero lst = if (null result) then [0] else result
    where result = filter (/= 0) lst

-- prepends each element to all given lists
-- addAll [1, 2] [[3, 4], [5, 6]] => [[1, 3, 4], [1, 5, 6], [2, 3, 4], [2, 5, 6]]
addAll :: [a] -> [[a]] -> [[a]]
addAll els lsts = concat [map (el:) lsts | el <- els]
 

-- nub removes duplicates 


buildKey :: [[Int]] -> [[Int]]
buildKey [[]] = [[]]
buildKey keys =
    if allNonEmpty then
        addAll uniques (buildKey rest)
    else
        [[]]
    where allNonEmpty = all (not . null) keys
          uniques = nub (filterOrZero (map head keys))
          rest = map tail keys


allPossibleKeys = buildKey (keys1 ++ keys2)


--tryDecrypt' :: [Int] -> [[Int]] -> String
tryDecrypt' binary keys = filter printable decryptions
                        where 
                            printable = all isPrint
                            decryptions = map (decryptBinary binary) keys

--tryDecrypt :: String -> [[Int]] -> String
tryDecrypt hex keys = tryDecrypt' (hexByteStringToInt hex) keys


answer = tryDecrypt targ allPossibleKeys

-- findSpaces 
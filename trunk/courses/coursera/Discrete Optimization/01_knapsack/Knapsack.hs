...
solve (n:k:rest) = knapsack n k (groupItems rest)

knapsack :: Integer -> Weight -> [Item] -> (Integer, [Integer])
knapsack = dynKnapsackBuild
...
--dynKnapsack = Integer -> Value -> [Item] -> Integer
--dynKnapsack n k items = dynKnapsack' k items (replicate n 0)

--dynKnapsack' k items solution
